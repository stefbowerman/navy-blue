...
const stylePipeline = (src) => {
  const paths = {
    src: path.join(config.root.src, config.tasks.notifications.src, src),
    dest: path.join(config.root.src, config.tasks.notifications.dest)
  };
...
gulp.task('notifications', () => mergeStream.apply(gulp, config.tasks.notifications.files.map(stylePipeline)));
